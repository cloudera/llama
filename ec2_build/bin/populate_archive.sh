#!/bin/bash
# To be called after build has been copied to archive/nightly. Populates, well, everything.
...
sudo -E -u www-data GNUPGHOME=$BASE_DIR/apt/gpg-home/ $GPG_AGENT_CRAP $BASE_DIR/apt/update_repo.sh -b $BUILD_ID -c cdh3 -r /var/www/archive_public/debian -d $BASE_DIR -u $UPDATE
...
sudo -E -u www-data GNUPGHOME=$BASE_DIR/yum/gnupg $GPG_AGENT_CRAP $BASE_DIR/yum/update_repos.sh -s $BASE_DIR/$BUILD_ID/ -c 3 -r /var/www/archive_public/redhat/ -p abf9a621527cfbaebd0d7f2c23994dd8 -u $UPDATE
sudo -E -u www-data GNUPGHOME=$BASE_DIR/yum/gnupg $GPG_AGENT_CRAP $BASE_DIR/yum/update_repos.sh -s $BASE_DIR/$BUILD_ID/ -c 3 -a x86_64 -r /var/www/archive_public/sles/11/x86_64/ -p abf9a621527cfbaebd0d7f2c23994dd8 -d sles11 -u $UPDATE
sudo -E -u www-data GNUPGHOME=$BASE_DIR/yum/gnupg $GPG_AGENT_CRAP $BASE_DIR/yum/update_repos.sh -s $BASE_DIR/$BUILD_ID/ -c 3 -a x86_64 -r /var/www/archive_public/redhat/6/x86_64/ -p abf9a621527cfbaebd0d7f2c23994dd8 -d rhel6 -u $UPDATE
...
sudo -E -u www-data /bin/bash finalize-staging.sh -b $BUILD_ID -c 3 -r /var/www/archive_public/ -d $BASE_DIR
