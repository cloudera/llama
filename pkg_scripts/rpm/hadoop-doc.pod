...
=item Note to Hadoop System Admins

Hadoop users can skip this information.

The C</etc/hadoop/config> link is managed by the alternatives(8) command.  You should B<not> change this
symlink directly.  Instead, use Hadoop configuration RPMs (highly recommended)
or run the alternatives(8) command from the commandline.  
...
To add your custom configuration, run the following commands as root:

 # cp -r /etc/hadoop/conf.empty /etc/hadoop/conf.my
 # cp /etc/init.d/hadoop-conf-empty /etc/init.d/hadoop-conf-my

This will create a new empty configuration and empty service description.  
Edit the configuration files in C</etc/hadoop/conf.my> and C</etc/init.d/hadoop-conf-my> to taste.
...
You now have a custom configuration with a priority of 90 activated.  The services in your C</etc/init.d/hadoop-conf-my>
service file will start on boot with the configuration in C</etc/hadoop/conf>.
